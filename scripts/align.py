...
import inspect
from eegmusic.models.eeg_encoder import EEGEncoderForAlign, EEGEncoderConfig, EEGEncoderForAlignLn, EEGEncoderForAlignFourier
from eegmusic.models.labram import LaBraMForAlign
from eegmusic.models.whisper import WhisperEncoder, CLAPEncoder
...
from eegmusic.datasets.nmed import NMEDDataset_h, NMEDDataset_t, CustomSampler
from eegmusic.utils.misc import augment_ts
from eegmusic.models.stat_model import AudioPreprocessor
...
def validate(model, val_loader, accelerator, global_step):
    ...
    for batch in val_loader:
        eeg = batch['eeg']
        music = batch['music']
        output = model(eeg, music, channel_dropout=0.0)
        loss_list.append(output.loss.item())
        acc_list.append(output.acc.item())
    accelerator.log({"val_loss": np.mean(loss_list)}, step=global_step)
    accelerator.log({"val_acc": np.mean(acc_list)}, step=global_step)
    return {"val_loss": np.mean(loss_list), "val_acc": np.mean(acc_list)}
...
def main(
        pretrain_model_path: str = "eegmusic/eeg-music-pretrain-h-t",
        ...
        use_fourier_model: bool = False,
        use_labram_model: bool = False,
        use_adam: bool = False,
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            ...
            "wandb": {
                ...
                'save_code': True,
                'notes': pbs_job_id,
                "name": f"{model_type}-{data_split_type}"
                }
            },
        )
    ...
    if model_type == 'labram':
        eeg_encoder = LaBraMForAlign.from_pretrained(pretrain_model_path)
    elif model_type == 'linear':
        eeg_encoder = EEGEncoderForAlignLn.from_pretrained(pretrain_model_path)
        # eeg_encoder = EEGEncoderForAlignLn(EEGEncoderConfig.from_pretrained(pretrain_model_path))
    elif model_type == 'fourier':
        eeg_encoder = EEGEncoderForAlignFourier.from_pretrained(pretrain_model_path)
    else:
        eeg_encoder = EEGEncoderForAlign.from_pretrained(pretrain_model_path)

    print(f"model_type: {model_type}")

    use_clap = isinstance(eeg_encoder.whisper, CLAPEncoder)
    print(f"use_clap: {use_clap}")

    music_sr = 48000 if use_clap else 16000
    audio_preprocessor = AudioPreprocessor(use_clap=use_clap)
    print(f"Splitting dataset with {data_split_type} split type")
    if data_split_type == 'default':
        dataset = ConcatDataset([NMEDDataset_h(data_path=os.path.join(data_path, 'H'), 
                                            ...
                                            audio_preprocessor=audio_preprocessor, global_crop_scale=1, music_sr=music_sr),
    ...
    elif data_split_type == 'subject':
        test_subs = ['S02_t', 'S03_t', 'S04_t', 'S05_t']
        train_subs = ['S06_t', 'S07_t', 'S08_t', 'S09_t', 'S10_t', 'S11_t', 
                    'S12_t', 'S13_t', 'S14_t', 'S15_t', 'S16_t', 'S17_t', 'S19_t', 'S20_t', 'S21_t', 'S23_t']
        train_subset = ConcatDataset([NMEDDataset_h(data_path=os.path.join(data_path, 'H'), 
                                            window_size=eeg_window_size, stride=eeg_window_stride, subject=None,
                                            audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr),
                            NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=train_subs,
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr)])
        test_subset = NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=test_subs,
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=1, music_sr=music_sr)
    elif data_split_type == 'song_intra':
        train_subset = ConcatDataset([NMEDDataset_h(data_path=os.path.join(data_path, 'H'), 
                                            window_size=eeg_window_size, stride=eeg_window_stride, subject=None, eeg_trim=[0, 0.95],
                                            audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr),
                            NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=None, eeg_trim=[0, 0.95],
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr)])
        test_subset = ConcatDataset([NMEDDataset_h(data_path=os.path.join(data_path, 'H'), 
                                            window_size=eeg_window_size, stride=eeg_window_stride, subject=None, eeg_trim=[0.95, 1],
                                            audio_preprocessor=audio_preprocessor, global_crop_scale=1, music_sr=music_sr),
                            NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=None, eeg_trim=[0.95, 1],
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=1, music_sr=music_sr)])
    elif data_split_type == 'song_inter':
        test_song_list = [21, 22]
        train_song_list = [23, 24, 25, 26, 27, 28, 29, 30]
        train_subset = ConcatDataset([NMEDDataset_h(data_path=os.path.join(data_path, 'H'), 
                                            window_size=eeg_window_size, stride=eeg_window_stride, subject=None,
                                            audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr),
                            NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=None, song_list=train_song_list,
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=0.7, music_sr=music_sr)])
        test_subset = NMEDDataset_t(data_path=os.path.join(data_path, 'T'), 
                                        window_size=eeg_window_size, stride=eeg_window_stride, subject=None, song_list=test_song_list,
                                        audio_preprocessor=audio_preprocessor, global_crop_scale=1, music_sr=music_sr)
    ...
    # sampler = CustomSampler(test_subset, seed=seed, shuffle=False)
    ...
    print(f"train_dataset size: {len(train_subset)}")
    print(f"val_dataset size: {len(test_subset)}")
    ...
    # encoder_params = []
    # proj_params = []
    # for name, param in eeg_encoder.named_parameters():
    #     if param.requires_grad:
    #         if "aligned_space_proj" in name:
    #             proj_params.append(param)
    #         else:
    #             encoder_params.append(param)
    # params = [
    #     {"params": encoder_params, "lr": learning_rate / 2},
    #     {"params": proj_params, "lr": learning_rate }
    # ]
    # if use_linear_model or use_fourier_model or use_adam:
    # if use_linear_model or use_adam:
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with accelerator.accumulate(eeg_encoder):
                eeg = augment_ts(batch['eeg'], crop_scale=crop_scale, 
                                              resize_len=eeg_window_size, noise_level=noise_level)
                music = batch['music']
                music_prepocessed = batch['music_preprocessed_global']
       
                output = eeg_encoder(eeg, music, music_prepocessed, channel_dropout=channel_dropout)
                loss = output.loss
